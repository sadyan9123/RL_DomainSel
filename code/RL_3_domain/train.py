...
TRAINING_SET_DIR = '/home/yl/a_domainsel_124/oct_traindata2'  # 训练集文件夹的地址
#DEBUG_TRAINING_SET = True  # 对训练集进行debug
#REPORT_REDUNDENT = True     # 是否报告冗余项
DEBUG_TRAINING_SET = False  # 对训练集进行debug
REPORT_REDUNDENT = False     # 是否报告冗余项
SHOW_PLOT = False
FEATURE_FILE_DIM = 25  
FEATURE_DOMAIN_SEL_DIM_EXT = 1  
FEATURE_DOMAIN_SEL_DIM_ORG = 93
...
if DEBUG_TRAINING_SET:
    ...
    FEATURE_DOMAINSEL_NAME = open('FEATURE_OCT-VA-PA_93.txt').readlines()
...
# 把int型的action改成vector [0,0,1][0,1,0][0,0,1]
def proc_act(act):
    if act == -1:
        return None
    ret = np.zeros(3)
...
def train(trans_seq, nn_arch,gamma, epoch,nn_extra,batch_size):
    ...
    ACTIONS_NUM = 3  # number of valid actions
...
# done dt+1 = dt, rstt+1 = safe/unsafe
# good d-t+1 = dt, rstt+1 = uncertain
# bad d−t+1 6= dt
# normal rstt+1 = unknown

#normal就是even，good就是just_alive，bad就是bad_choice，done就是succ_done
