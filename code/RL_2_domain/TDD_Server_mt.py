...
if NN_dir != 'None':
    ...
    if args.is_full_feature:
        StateShape = [None, 345, 1] 
    else:
        StateShape = [None, 106, 1]  
    print("新建一个ForwardNet")
    net = NN(StateShape, 2, extra)
...
class MyBaseRequestHandlerr(BaseRequestHandler):
    ...
    def get(self):
        ...
        if self.target_fn is None:
            fn = dict_obj['src']
            # self.response_type = 'prefix' if 'f1' in dict_obj else 'domain'
            # 目前只做domain
            self.response_type = 'domain'
            assert fn is not None
            assert self.target_fn is None

            dir = './dump_' + self.response_type + '_' + server_start_time

            # if not os.path.exists(dir):
            #     os.system('mkdir ' + dir)

            self.target_fn = dir + '/' + fn + '.' + str(self.conn_time) + '.' + self.response_type
	    
            #print('%r -> %r' % (self.client_address, self.target_fn))
            sys.stdout.flush()
    ...
    def send_random_domain(self):
        ...
        rand_bool = random.randrange(2) == 0
        domain_name = 'predicate' if rand_bool else "value"
        domain = 1 if rand_bool else 0
    ...
    def send_QL_domain(self):
        ...
        domain_name = 'predicate' if domain==1 else "value"
        dict_obj = {"type": "action_domainsel", "action_name": domain_name, 'action': domain}
        self.send(dict_obj)
